...
ui: check-env build setup-kubeconfig setup-ui-reports
	...
	$(DOCKER_RUN) $(DOCKER_VOLUMES_UI) \
		$(ENV_FILE_FLAG) \
		-e KUBECONFIG=/kubeconfig \
		glueops-tests -m oauth_redirect tests/ui/ -vv --reruns 2 --reruns-delay 5 \
	...
	$(DOCKER_RUN) $(DOCKER_VOLUMES_UI) \
		$(ENV_FILE_FLAG) \
		-e KUBECONFIG=/kubeconfig \
		glueops-tests -m authenticated tests/ui/ -vv --reruns 2 --reruns-delay 5 \
...
full: check-env build setup-kubeconfig setup-reports setup-ui-reports
	...
	$(DOCKER_RUN) $(DOCKER_VOLUMES) \
		$(ENV_FILE_FLAG) \
		-e KUBECONFIG=/kubeconfig \
		glueops-tests -vv --reruns 2 --reruns-delay 5 \
